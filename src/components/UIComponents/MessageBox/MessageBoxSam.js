...
class MessageBoxSam extends Component {
constructor(props) {
  super(props);
  this.state={
...
componentDidMount(){
 ...
 <div className="input-container">
         ...
         <select className="input-scan"  placeholder="" onChange={this.resChange}>
           <option>Select a Resolution</option>

           {JSON.parse(localStorage.getItem('resolution')).map((name,ind) => <option key={name} value={name}>{name}</option>)}
